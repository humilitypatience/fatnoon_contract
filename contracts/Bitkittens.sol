...
contract Bitkittens is ERC721Enumerable, Ownable{
    ...
    uint private addPriceIncrease = 0.1 ether;
    uint private max_Bitkittens = 300;
    ...
    uint256 private reservedPeriod = 30 * 60; //24 * 3600;               // 24 hours
    uint256 private mintReservedTokenPeriod = 30 * 60; //24 * 3600;      // 24 hours
    uint256 private initialTime;
    uint256 private initialReservePrice = 0.01 ether;
...
//      _baseTokenURI = "https://gateway.pinata.cloud/ipfs/Qme8xVAFXMXLtuofRnjWF57mpwHqTW8czsT5MXNDuvVvWA/";
        
        // addressOfPumpkitten = 0x250D768BdA070c21e0Da110d8Ef52317A917c6E5;
    ...
    function buyKittens() payable public {
        require(_tokenIdTracker.current() <= max_Bitkittens, "Too Many Tokens");
        ...
        {
            uint length = _reservedInfo[_msgSender()].tokenId.length;
        ...
        if (enableOGSharedistribute)
        {
            ...
            if (_tokenIdTracker.current() % 5 == 0)     // 50
            {
                require(amountOfOGShareDistribute < address(this).balance, "Balance is insufficient");
                
                if (length >= 0)
                {
                    address holder;
                    uint256 amount = amountOfOGShareDistribute / length;
            
                    for (uint i=0; i<length; i++)
                    {
                        holder = ERC721(addressOfPumpkitten).ownerOf(i + 1);
                        
                        if (!_excludedHolders[holder])
                        {
                            payable(holder).transfer(amount);
                        }
                    }
            
                    amountOfOGShareDistribute = 0;
                }
            }
        }
    ...
    function setExcludedListOGShareDistribute(address account, bool exclude) public onlyOwner {
        _excludedHolders[account] = exclude;
    }
    ...
    function addReserveToken(address account) public onlyOwner {
        ...
        require(currentReservedTokenId <= max_Bitkittens - max_TokenCountOfAccount, "Too Many Tokens");
    ...
    function getPrice() public view returns(uint) {
        if (_tokenIdTracker.current() == max_Bitkittens)
            return 0;
    ...
    function setMaxBitkittens(uint amount) public onlyOwner {
        max_Bitkittens = amount;
    }
