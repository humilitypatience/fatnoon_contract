...
contract Samuraikittens is ERC721Enumerable, Ownable{
    using Counters for Counters.Counter;
    
    string private _baseTokenURI;
    uint private reservePrice = 0.3 ether;
    uint public currentPrice;
    uint public previousPrice;
    uint private addPriceIncrease = 0.3 ether;
    uint private max_Kittens = 500;
    uint256 private max_TokenCountOfAccount = 2;
    
    uint private mintTaxRateForDeveloper = 5000;            // 50%
    bool private enableMintTaxForDeveloper = true;
    
    address private devAccount;

    bool private enableDistributeShare = true;
    ...
    uint256 private reservedPeriod = 24 * 3600;        // 24 hours
    uint256 private mintReservedTokenPeriod = 24 * 3600;        // 24 hours
    uint256 private initialTime;
    uint256 private initialReservePrice = 0.449 ether;

    uint private rateOGShareDistribute = 50;                                // 0.5%
    ...
    address private addressOfPumpkitten;
    
    mapping (address => uint256) private _claimedFTM;
    
    uint private unlocked = 1;
    modifier lock() {
        require(unlocked == 1, 'SamuraiKittens: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }

    constructor(address _pkaddress) ERC721
        ("Samuraikittens", "SK") 
    {
//        _baseTokenURI = "https://gateway.pinata.cloud/ipfs/QmXMvgmj9aNxymSvbB3m9CKkjFUTqS5t2H7FUVobpe2E64/";
//        addressOfPumpkitten = 0x31EC182dfBc6B6B19D68419C0500067Ce000318d;
    
       _baseTokenURI = "https://gateway.pinata.cloud/ipfs/QmUV2B2RrxAwX4Hx4Y2mr9HVnAVoEgvQVtmfbbqgrcfiFJ/";
        addressOfPumpkitten = _pkaddress;
    ...
    function buyKittens() payable public lock {
        ...
        if ((block.timestamp - initialTime) <= mintReservedTokenPeriod)
        {
            ...
            {
                if (_tokenStatus[_reservedInfo[_msgSender()].tokenId[i]] == Status.Minted)
                    continue;
                ...
                _mint(_msgSender(), _reservedInfo[_msgSender()].tokenId[i]);
                
                _tokenCountOfAccount[_msgSender()] ++;
                
                _tokenStatus[_reservedInfo[_msgSender()].tokenId[i]] = Status.Minted;
                _tokenPrice[_reservedInfo[_msgSender()].tokenId[i]] = _reservedInfo[_msgSender()].price[i];
            ...
            console.log("initialReservePrice:", initialReservePrice);

            if (initialReservePrice > 0)
            {
                currentPrice = initialReservePrice;
                initialReservePrice = 0;
            }
            
            if (_tokenIdTracker.current() < currentReservedTokenId)
            {
                uint i;
                for (i=1; i<currentReservedTokenId + 1; i++)
                {
...
//          currentPrice = previousPrice + addPriceIncrease;
        ...
        uint256 mintTaxAmount;
        if (mintTaxRateForDeveloper > 0 && enableMintTaxForDeveloper && devAccount != address(0))
        {
            mintTaxAmount = previousPrice * mintTaxRateForDeveloper / (10000);
            
            require(payable(devAccount).send(mintTaxAmount));
        }
        ...
        if (enableOGSharedistribute)
        {
            ...
            amountOfOGShareDistribute += previousPrice * rateOGShareDistribute * length / (10000);
        }
    ...
    function pendingFTM() public view returns (uint256){
        if (_excludedHolders[msg.sender] == true )
            return 0;
            
        uint balance = ERC721Enumerable(addressOfPumpkitten).balanceOf(msg.sender);
        if (balance == 0)
            return 0;
            
        uint length = ERC721Enumerable(addressOfPumpkitten).totalSupply();
        uint256 amountOfHolder = amountOfOGShareDistribute * balance / length - _claimedFTM[msg.sender];
        
        return amountOfHolder;
    }
    ...
    function claimFTM() external lock {
        ...
        uint balance = ERC721Enumerable(addressOfPumpkitten).balanceOf(msg.sender);
        require(balance > 0, 'You are not a holder of OG Pumpkittens.');
        
        uint length = ERC721Enumerable(addressOfPumpkitten).totalSupply();
        uint256 amountOfHolder = amountOfOGShareDistribute * balance / length - _claimedFTM[msg.sender];
        
        require(amountOfHolder > 0, "Balance is not zero");
        
        require(amountOfHolder < address(this).balance, "Insufficient balance.");
        
        require(payable(msg.sender).send(amountOfHolder));
            
        _claimedFTM[msg.sender] += amountOfHolder;
    }
    ...
    function setExcludedListOGShareDistribute(uint tokenIdofPumpkitten) public onlyOwner {
        address account = ERC721Enumerable(addressOfPumpkitten).ownerOf(tokenIdofPumpkitten);
        _excludedHolders[account] = true;
    }

    function addReserveToken(address account) public onlyOwner {
        require((block.timestamp - initialTime) < reservedPeriod, "End time");
        require(_reservedInfo[account].tokenId.length < max_TokenCountOfAccount, "Too Many Tokens");
        require(currentReservedTokenId <= max_Kittens - max_TokenCountOfAccount, "Too Many Tokens");
        
        uint length = _reservedInfo[account].tokenId.length;

        for (uint i=length; i<max_TokenCountOfAccount; i++)
        {
            _reservedInfo[account].price.push(currentPrice);
            
            currentReservedTokenId++;
            _reservedInfo[account].tokenId.push(currentReservedTokenId);
    ...
    function setInitialReservePrice(uint256 _price) public onlyOwner {
        require(_price > 0, "Price is wrong");
        initialReservePrice = _price;
    }
    ...
    function setMintTaxRateForDeveloper(uint _mintTaxRate) public onlyOwner {
        require(_mintTaxRate > 0, "Cannot be zero value");
        mintTaxRateForDeveloper = _mintTaxRate;
    }
    
    function setEnableMintTax(bool _enableMintTax) public onlyOwner {
        enableMintTaxForDeveloper = _enableMintTax;
    }
